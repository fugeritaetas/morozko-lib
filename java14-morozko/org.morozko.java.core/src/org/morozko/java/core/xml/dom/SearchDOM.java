 ...
 * @package     : org.morozko.java.core.xml.dom
 * @creation	: 13/lug/07
 * @release		: xxxx.xx.xx
 */
package org.morozko.java.core.xml.dom;
...
public class SearchDOM {
    ...
    private static List makeList(Element base) {
    ...
    public String findText(Element node) {
        String text = null;
        List list = this.findAllText(node);
        if (!list.isEmpty()) {
    ...
    public List findAllText(Element node) {
    ...
    public List findAllTags(Element node, String name) {
        List result = new ArrayList();
        List search = makeList(node);
    ...
    private Element findTag(List queue, String name) {
        Element tag = null;
