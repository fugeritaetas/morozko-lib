		...
		try {
			...
			if ( bundleListTag != null ) {
				...
				while ( bundleIt.hasNext() ) {
					...
					this.getLog().info( "bundle name : "+name );
					this.getLog().info( "bundle path : "+path );
					try {
						...
						this.getLog().error( "Failed bundle setup", e );
					}
				}
			}
			// filter list
			...
			if ( filterListTag != null ) {
				...
				while ( filterIt.hasNext() ) {
					...
					this.getLog().info( "filter name : "+name );
					this.getLog().info( "filter type : "+type );
					try {
