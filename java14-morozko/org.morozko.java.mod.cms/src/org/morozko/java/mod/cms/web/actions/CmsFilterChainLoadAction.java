 ...
 *
 */
public class CmsFilterChainLoadAction extends AbstractAction {
	...
	public ActionForward doExecute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String idCmsFilterChain = findParameter( request , "idCmsFilterChain" );
		if ( idCmsFilterChain != null ) {
			CmsFilterChainDAO cmsFilterChainDAO = CmsDAOFactory.getInstance().getCmsCoreDAOFactory().getCmsFilterChainDAO();
			request.getSession().setAttribute( "cmsFilterChainModel" , cmsFilterChainDAO.loadOneCmsFilterChainPk( DAOID.valueOf( idCmsFilterChain ) ) );
		} else {
			request.getSession().removeAttribute( "cmsFilterChainForm" );
			request.getSession().removeAttribute( "cmsFilterChainModel" );
		}
